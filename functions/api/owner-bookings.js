...
export async function onRequestGet({request,env}){
  ...
  const owner=await getOwnerSession(request,env);if(!owner||owner.type!=='owner')return jsonResponse({error:'Unauthorized'},401,request);
  ...
  try{
    ...
    return jsonResponse(bookings.filter(b=>ids.includes(String(b.homestayId))),200,request,{'Cache-Control':'no-store'});
  }catch(e){console.error('Owner bookings error:',e.message);return jsonResponse({error:'Failed to load bookings'},500,request)}
}
