import { corsHeaders, enforceHttps, getClientIP, getGuestSession, jsonResponse, logAction } from './_utils.js';

export async function onRequestPost({ request, env }) {
  const redirect=enforceHttps(request); if(redirect)return redirect;
  try {
    const session=await getGuestSession(request,env);
    if(!session||session.type!=='guest')return jsonResponse({error:'Authentication required'},401,request);
    ...
    if(String(booking.guestId)!==String(session.userId))return jsonResponse({error:'Unauthorized'},403,request);
    if(booking.status!=='Pending Payment')return jsonResponse({error:`Booking is not awaiting payment. Current status: ${booking.status}`},409,request);
    if(booking.toyyibpay_billcode)return jsonResponse({success:true,billCode:booking.toyyibpay_billcode,url:`https://toyyibpay.com/${booking.toyyibpay_billcode}`,bookingId,amount:Number(booking.total)},200,request);
    ...
    if(!live)return jsonResponse({error:'Payment gateway is not enabled. Set TOYYIBPAY_PAYMENT_ENABLED=true and configure the ToyyibPay keys.'},503,request);
    const domain=env.PUBLIC_DOMAIN||new URL(request.url).origin;
    const form=new FormData();
    form.append('userSecretKey',secret);form.append('categoryCode',category);
    ...
    form.append('billPriceSetting','1');form.append('billPayorInfo','1');
    ...
    form.append('billSplitPayment','0');form.append('billPaymentChannel','0');form.append('billDisplayMerchant','1');
    const res=await fetch('https://toyyibpay.com/index.php/api/createBill',{method:'POST',body:form});
    const data=await res.json().catch(()=>null);
    if(!res.ok||!data?.[0]?.BillCode)return jsonResponse({error:'ToyyibPay bill creation failed'},502,request);
    const billCode=String(data[0].BillCode);
    bookings[idx]={...booking,toyyibpay_billcode:billCode,toyyibpay_created_at:new Date().toISOString(),paymentProvider:'ToyyibPay'};
    await db.prepare('INSERT OR REPLACE INTO store(key,data) VALUES(?,?)').bind('kd_bookings',JSON.stringify(bookings)).run();
    await logAction({db,action:'toyyibpay_bill_created',admin:'guest',details:`Bill ${billCode} created for ${booking.id}`,ip:getClientIP(request),userId:session.userId,homestayId:booking.homestayId});
    return jsonResponse({success:true,url:`https://toyyibpay.com/${billCode}`,id:billCode,billCode,bookingId:booking.id,amount:Number(booking.total)},200,request);
  }catch(e){console.error('ToyyibPay create error:',e.message);return jsonResponse({error:'Payment setup failed'},500,request)}
}
