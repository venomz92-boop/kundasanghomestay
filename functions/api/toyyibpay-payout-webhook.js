...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    // Only process successful payouts
    ...
    if (!referenceNo) {
      ...
      return new Response('Missing referenceNo', { 
        status: 400, 
        headers: corsHeaders(request) 
      });
    }
    ...
    if (idx !== -1) {
      // Check if already marked as payout success
      ...
      bookings[idx].status = "Completed - Payout Success RM" + (amount || bookings[idx].payoutAmount || '');
      ...
      return new Response(`Booking ${referenceNo} not found`, { 
        status: 404, 
        headers: corsHeaders(request) 
      });
    }
...
export async function onRequestGet({ request }){ 
  return new Response("ToyyibPay Payout webhook ready", { 
    status: 200, 
    headers: corsHeaders(request) 
  }); 
}
