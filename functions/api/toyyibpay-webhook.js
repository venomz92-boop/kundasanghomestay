...
export async function onRequestPost({request,env}){
  ...
  try{
    ...
    if(type.includes('application/json')) data=await request.json(); else {const form=await request.formData();for(const [k,v] of form.entries())data[k]=String(v)}
    const status=String(data.status||''), orderId=String(data.order_id||''), refno=String(data.refno||''), billcode=String(data.billcode||''), receivedHash=String(data.hash||'');
    if(!orderId||!billcode||!receivedHash)return new Response('invalid callback',{status:400,headers:corsHeaders(request)});
    const secret=env.TOYYIBPAY_SECRET_KEY;if(!secret)return new Response('server not configured',{status:500,headers:corsHeaders(request)});
...
export async function onRequestGet(){return new Response('ToyyibPay webhook endpoint',{status:200})}
