// /api/toyyibpay-webhook.js – with email on payment success
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (status === '1') {
      ...
      const emailSent = await sendCheckinCodeEmail(
        booking.guestEmail,
      ...
      if (emailSent) {
        ...
        console.warn(`⚠️ Failed to send check‑in code email to ${booking.guestEmail}`);
      }
    } else if (status === '3') {
      bookings[idx] = {
        ...booking,
